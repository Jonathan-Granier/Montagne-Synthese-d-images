...
vec3 grid_color(vec2 coord, float grid_amplitude){
    int number=0;

    coord.x = abs(coord.x);
    coord.y = abs(coord.y);

    while(coord.x > grid_amplitude)
    {
        number ++;
        coord.x -= grid_amplitude;
    }
    while(coord.y > grid_amplitude)
    {
        number ++;
        coord.y -= grid_amplitude;
    }
    if(number%2==0)
        return vec3(1,1,1);
    return vec3(0,0,0);
...
{
    vec2 focal_center = vec2(0.5,0.5);
    float seuil_uniform_light = 0.25;
    float transition_uniform_to_rainbow = 0.00;
    float seuil_rainbow = seuil_uniform_light + transition_uniform_to_rainbow;
    float rainbow_length = 0.1;
    float seuil_grid = seuil_rainbow + rainbow_length;
    float grid_amplitude = 0.15;
    ...
    return grid_color(coord, grid_amplitude);
}
